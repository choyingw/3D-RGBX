...
class CoarseMatching(nn.Module):
    ...
    def forward(self, feat_c0, feat_c1, data, mask_c0=None, mask_c1=None):
        ...
        # print(diag.max(), diag.min(), diag.dtype, diag.shape)
        ...
        if self.inference:
            ...
            data.update({"conf_matrix_0_to_1": conf_matrix_0_to_1, "conf_matrix_1_to_0": conf_matrix_1_to_0})
            # predict coarse matches from conf_matrix
            data.update(**self.get_coarse_match_training(conf_matrix_0_to_1, conf_matrix_1_to_0, data))

    def forward_sim(self, feat_c0, feat_c1, data, mask_c0=None, mask_c1=None):
        feat_c0 = self.final_proj(feat_c0)
        feat_c1 = self.final_proj(feat_c1)
    ...
    def get_coarse_match_training(self, conf_matrix_0_to_1, conf_matrix_1_to_0, data):
        ...
        mkpts0_c = (
            torch.stack([i_ids % data["hw0_c"][1], torch.div(i_ids, data["hw0_c"][1], rounding_mode="trunc")], dim=1)
            * scale0
        )
        mkpts1_c = (
            torch.stack([j_ids % data["hw1_c"][1], torch.div(j_ids, data["hw1_c"][1], rounding_mode="trunc")], dim=1)
            * scale1
        )
    ...
    def get_coarse_match_inference(self, sim_matrix, data):
        ...
        mask = torch.logical_or(
            mask, (conf_matrix_ > self.thr) * (conf_matrix_ == conf_matrix_.max(dim=1, keepdim=True)[0])
        )
