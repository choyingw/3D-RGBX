...
PATTERN_IDS = {"random": 0, "velodyne": 1, "sfm": 2}
...
class BaseDataset(Dataset):
    ...
    def process(self, rgb, dep, K, normalize_median=True, random_crop=True):
        ...
        if self.augment and self.mode == "train":
            ...
            if random_crop:
                # Random crop
                i, j, h, w = T.RandomCrop.get_params(rgb, output_size=self.crop_size)
            ...
            t_dep = T.Compose([self.ToNumpy(), T.ToTensor()])
            ...
            t_rgb_np_raw = T.Compose(
                [T.Resize((self.height, self.width)), T.CenterCrop(self.crop_size), self.ToNumpy()]
            )
    ...
    def get_sparse_depth(self, dep, pattern_raw, match_density=True, rgb_np=None, dep_np=None, input_noise="0.0"):
        dep = torch.clone(dep)
        ...
        if pattern.isdigit():
            ...
            points2D_target_valid = points2D_target[
                :,
                (
                    (points2D_target[0] >= 0)
            ...
            # N_valid
            depth_target_valid = depth_target[
                (
                    (points2D_target[0] >= 0)
                    & (points2D_target[0] < width_expanded)
                    & (points2D_target[1] >= 0)
            ...
            if len(v) == 0:
                return self.get_sparse_depth(
                    dep, "1000", match_density=match_density, rgb_np=rgb_np, input_noise=input_noise
                )
            ...
            points2D_source_valid = points2D_source[
                :,
                (
                    (points2D_source[0] >= 0)
            ...
            # N_valid
            depth_source_valid = depth_source[
                (
                    (points2D_source[0] >= 0)
                    & (points2D_source[0] < width)
                    & (points2D_source[1] >= 0)
            ...
            if pattern == "sift":
                ...
                detector = cv2.ORB_create(
                    nfeatures=100000,  # max keypoints to retain
                    scaleFactor=1.05,  # image pyramid decimation
                    nlevels=8,  # pyramid levels
                    edgeThreshold=2,  # size of border where features are not detected
                    firstLevel=0,
                    WTA_K=4,  # 2, 3, or 4 (bits compared, affects descriptor)
            ...
            if random.random() < 0.9:  # 90% time use depth detector
                keypoints = detector.detect(dep_np)
            else:
                keypoints = detector.detect(gray)
            ...
            for keypoint in keypoints:
                if random.random() < value:  # dropout by sampled threshold
