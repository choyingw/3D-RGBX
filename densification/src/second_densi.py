...
def test(args):
    ...
    for ori, fil in tqdm(zip(ori_li, li), total=min(len(ori_li), len(li)), desc="densification second"):
        ...
        img_array = np.array(rgb_).astype(np.float32)
        noise = np.random.normal(0, 0.02, img_array.shape)
        noisy_array = np.clip(img_array + noise, 0, 255).astype(np.uint8)
        rgb_ = Image.fromarray(noisy_array)

        rgb = t_rgb(rgb_)
        dep_sp = np.load(fil)
        if load_ori:
            dep_sp_mask = dep_sp == 0
            dep_ori = np.load(ori)

            # background_mask = (dep_ori == 0).astype(np.uint8)
            ...
            # dep_ori[dilated_bg == 1] = 0

            # dep_ori = cv2.resize(dep_ori.astype(np.uint8), (518, 294), interpolation=cv2.INTER_NEAREST)
            dep_ori_mask = dep_ori != 0
            dep_ori = dep_ori.astype(np.float32)

            # infill the area where filtered is void and original matching has values
            combined_mask = np.logical_and(dep_ori_mask, dep_sp_mask)
            true_indices = np.flatnonzero(combined_mask)
            n_disable = int(args.sample_rate * len(true_indices))
            disable_indices = np.random.choice(true_indices, size=n_disable, replace=False)
            combined_mask.flat[disable_indices] = False

            dep_sp[combined_mask] = dep_ori[combined_mask]

        m2 = dep_sp != 0
        flat_idx = np.flatnonzero(m2)  # same True entries as 1D flat indices
        rng = np.random.default_rng(0)  # set seed for reproducibility if you like
        k = int(len(flat_idx) * 0.4)  # "half" (floors if odd)
        drop = rng.choice(flat_idx, size=k, replace=False)
        m2.flat[drop] = False
        dep_sp[m2] = 0
