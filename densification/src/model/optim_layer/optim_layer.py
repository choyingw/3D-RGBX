...
class DepthGradOptimLayer(torch.autograd.Function):
    ...
    def forward(
        ctx,
        batched_image_gradients,
        batched_sparse_depth,
        batched_valid_sparse_mask,
        batched_confidence,
        batched_input_confidence,
        resolution=1,
        x_init=None,
        b_init=None,
        lamda=0.1,
        ...
        multires_batched_sparse_depth, multires_batched_valid_sparse_mask = multires_sparse_depth_downsample(
            batched_sparse_depth, batched_valid_sparse_mask, resolution
        )
        ...
        for i in range(1):
            ...
            b_top = b_top + (1.0 / r**2) * lamda[:, i : i + 1] * (
                batched_input_confidence_this_res * batched_valid_sparse_mask_this_res * batched_sparse_depth_this_res
            ).repeat_interleave(r, dim=2).repeat_interleave(r, dim=3).reshape(
                batch_size, -1, 1
            )  # B x (H*W) x 1
    ...
    def backward(ctx, gradx, grad_b_init):
        ...
        A = FastFiniteDiffMatrix(H, W, resolution, device=device, dtype=dtype)
        A_sparse = construct_diff_matrix_sparse(H, W, resolution, device=device, dtype=dtype).unsqueeze(
            0
        )  # B x num_eqns x (H*W)
        ...
        gradb, info = cg_batch(batched_RTRp, gradx, X0=grad_b_init, rtol=ctx.rtol, maxiter=ctx.max_iter, verbose=False)
        # print('backward:', 'stopped in %d steps' % info['niter'], 'resolution %dx%d' % (W, H))

        if torch.isnan(gradb).any():
            return (
                torch.zeros_like(batched_image_gradients),
                None,
                None,
                torch.zeros_like(batched_image_gradients),
                None,
        ...
        for i in range(1):
            ...
            term1 = (
                F.avg_pool2d((gradb).reshape(batch_size, 1, H, W), kernel_size=r)
                * batched_valid_sparse_mask_this_res
                * batched_sparse_depth_this_res
            )  # B x 1 x H/r x W/r
        ...
        return (
            grad_batched_image_gradients,
            None,
            None,
            grad_batched_confidence,
            grad_batched_input_confidence,
            None,
            None,
            grad_b_init_to_return,
            grad_lamda,
            None,
            None,
        )
