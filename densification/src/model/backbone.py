...
def convt_bn_relu(ch_in, ch_out, kernel, stride=1, padding=0, output_padding=0, bn=True, relu=True):
    ...
    layers = []
    layers.append(nn.ConvTranspose2d(ch_in, ch_out, kernel, stride, padding, output_padding, bias=not bn))
    if bn:
        layers.append(nn.BatchNorm2d(ch_out))
    if relu:
        layers.append(nn.ReLU(inplace=True))
...
class Backbone(nn.Module):
    def __init__(self, args, mode="rgbd", depth_input_channels=1):
        ...
        if mode == "rgbd":
            ...
            self.conv1_dep = conv_bn_relu(depth_input_channels, 16, kernel=3, stride=1, padding=1, bn=False)
            self.conv1 = conv_bn_relu(64, 64, kernel=3, stride=1, padding=1, bn=False)
        elif mode == "rgb":
        ...
        if args.backbone == "cformer":
            self.former = PVT(
                in_chans=64,
                patch_size=2,
                pretrained=Path(__file__).parent / "../pretrained/pvt.pth",
                backbone_pattern_condition_format=args.backbone_pattern_condition_format,
                num_pattern_types=args.num_pattern_types,
            )
        ...
        self.dec6 = nn.Sequential(
            convt_bn_relu(channels[5], 256, kernel=3, stride=2, padding=1, output_padding=1),
            BasicBlock(256, 256, stride=1, downsample=None, ratio=16),
        )
        # 1/8
        self.dec5 = nn.Sequential(
            convt_bn_relu(256 + channels[4], 128, kernel=3, stride=2, padding=1, output_padding=1),
            BasicBlock(128, 128, stride=1, downsample=None, ratio=8),
        )
        # 1/4
        self.dec4 = nn.Sequential(
            convt_bn_relu(128 + channels[3], 64, kernel=3, stride=2, padding=1, output_padding=1),
        ...
        if self.output_resolution <= 2 or self.args.prop_time > 0:
            ...
            self.dec3 = nn.Sequential(
                convt_bn_relu(64 + channels[2], 64, kernel=3, stride=2, padding=1, output_padding=1),
                BasicBlock(64, 64, stride=1, downsample=None, ratio=4),
            )
        ...
        if self.output_resolution <= 1 or self.args.prop_time > 0:
            ...
            self.dec2 = nn.Sequential(
                convt_bn_relu(64 + channels[1], 64, kernel=3, stride=2, padding=1, output_padding=1),
                BasicBlock(64, 64, stride=1, downsample=None, ratio=4),
            )
        ...
        if self.args.prop_time > 0:
            self.gd_dec1 = conv_bn_relu(64 + 64, 64, kernel=3, stride=1, padding=1)

            if self.args.spn_type == "dyspn":
                self.gd_dec0 = conv_bn_relu(
                    64 + 64, 5 * self.args.prop_time, kernel=3, stride=1, padding=1, bn=False, relu=False
                )
            elif self.args.spn_type == "nlspn":
                self.gd_dec0 = conv_bn_relu(
                    64 + 64, self.num_neighbors, kernel=3, stride=1, padding=1, bn=False, relu=False
                )
            else:
                raise NotImplementedError
        ...
        if self.args.prop_time > 0 and self.args.conf_prop:
            ...
            if self.args.spn_type == "dyspn":
                ...
                self.cf_dec0 = nn.Sequential(
                    nn.Conv2d(32 + 64, 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)), nn.Sigmoid()
                )
            else:
                raise NotImplementedError
        ...
        if self.args.pred_depth:
            ...
            # 1/1
            self.dep_dec1 = conv_bn_relu(64 + decoder_input_channel, 64, kernel=3, stride=1, padding=1)
            self.dep_dec0 = conv_bn_relu(
                64 + additional_decoder_channels, 1, kernel=3, stride=1, padding=1, bn=False, relu=True
            )
        ...
        if self.args.pred_context_feature:
            ...
            self.ctx_dec0 = conv_bn_relu(64, out_dim, kernel=3, stride=1, padding=1, bn=False, relu=False)
        ...
        if self.args.pred_confidence_input:
            ...
            self.cfi_dec0 = nn.Sequential(
                nn.Conv2d(32, 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)), nn.Sigmoid()
            )
