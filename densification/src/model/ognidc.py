...
def upsample_depth(depth, mask, r=8):
    ...
    N, C, H, W = depth.shape  # B x 1 x H x W
    mask = mask.view(N, C, 9, r, r, H, W)
    ...
    return up_depth.reshape(N, C, r * H, r * W)
...
class OGNIDC(nn.Module):
    def __init__(self, args):
        ...
        elif "NORMAL" in args.train_data_name:
        ...
        if args.spn_type == "nlspn":
            ...
            if self.prop_time > 0:
                self.prop_layer = NLSPN(args, self.num_neighbors, 1, 3, self.args.prop_kernel)
            ...
            if self.prop_time > 0:
                ...
                self.prop_layer = DySPN_Module(iteration=self.prop_time, num=self.num_neighbors, mode="yx")
        else:
    ...
    def forward(self, sample):
        ...
        if self.downsample_rate > 1:
            if self.args.depth_downsample_method == "mean":
                ...
                valid_sparse_mask = F.max_pool2d(
                    valid_sparse_mask, self.downsample_rate
                )  # mask is 1 if at least one pt in neighbor
        ...
        if self.args.training_depth_random_shift_range > 0.0 and self.training:
            ...
            random_shift = (
                torch.empty(batch_size).uniform_(-0.5, 0.5).cuda() * self.args.training_depth_random_shift_range
            )
        ...
        if self.args.load_dav2:
            if self.training:
                ...
                # resize back
                depth_pred_raw = F.interpolate(
                    depth_pred_raw, (H, W), mode="bilinear", align_corners=True
                )  # B x 1 x H x W
        ...
        _, spn_guide, spn_confidence, context, confidence_input, confidence_output = self.backbone(
            rgb, dep_network_input, depth_pattern
        )
        ...
        for itr in range(self.GRU_iters):
            ...
            # !!!HERE
            # confidence_input = confidence_input.repeat(3, 1, 1, 1)
            # weights_depth_grad = weights_depth_grad.repeat(3, 1, 1, 1)
            # up_mask = up_mask.repeat(3, 1, 1, 1)
            # weights_input = weights_input.repeat(3, 1, 1, 1)

            # !!!HERE
            # the optimization layer use the prediction from last round to accelerate convergence
            log_depth_pred, b_init = DepthGradOptimLayer.apply(
                log_depth_grad_pred,
                dep_integrator,
                valid_sparse_mask,
                weights_depth_grad,
            ...
            # !!!HERE
            # confidence_input = rearrange(confidence_input, "(b g) c h w -> b (g c) h w", g=3)
            ...
            if self.prop_time > 0 and (self.training or itr == self.GRU_iters - 1):
                if self.args.spn_type == "dyspn":
                    spn_out = self.prop_layer(depth_pred_up_init, spn_guide, dep_original, spn_confidence)
                    depth_pred_up_final = spn_out["pred"]
                    dyspn_offset = spn_out["offset"]
                elif self.args.spn_type == "nlspn":
                    depth_pred_up_final, _, _, _, _ = self.prop_layer(
                        depth_pred_up_init, spn_guide, spn_confidence, None
                    )
                    dyspn_offset = None
            else:
            ...
            # depth_predictions_up.append(depth_pred_up_init)
            # dyspn_offset = None

        output = {
            "pred": depth_predictions_up[-1],
            "pred_inter": depth_predictions_up,
