...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1, stride=1, bias=False
            ),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1, stride=1, bias=False
            ),
        )
...
class GlobalWeightsHead(nn.Module):
    def __init__(self, resolution, input_dim=64, hidden_dim=128):
        ...
        self.decoder = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, resolution))
...
class BasicUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, depth, depth_grad, K):
        ...
        if self.args.multi_resolution_learnable_gradients_weights == "learnable":
            ...
            weights_depth_grad = (
                weights_depth_grad.reshape(B, self.resolution, 1, 1).repeat(1, 1, H, W).repeat_interleave(2, dim=1)
            )
