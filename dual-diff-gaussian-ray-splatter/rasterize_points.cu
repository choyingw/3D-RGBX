	...
	const bool debug)
{
  ...
  torch::Tensor out_temperature = torch::full({NUM_CHANNELS_THERMAL, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		temperature.contiguous().data<float>(), 
		...
		out_temperature.contiguous().data<float>(),
		out_invdepthptr,
		antialiasing,
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_temperature, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_temperature,
	...
	const bool debug)
{
  ...
  torch::Tensor dL_dtemperature = torch::zeros({P, NUM_CHANNELS_THERMAL}, means3D.options());
  ...
  {  
	  ...
	  dL_dout_temperature.contiguous().data<float>(),
	  dL_dout_invdepthptr,
	  dL_dmeans2D.contiguous().data<float>(),
	  dL_dconic.contiguous().data<float>(),  
	  dL_dopacity.contiguous().data<float>(),
	  dL_dcolors.contiguous().data<float>(),
	  dL_dtemperature.contiguous().data<float>(),
	  dL_dinvdepthsptr,
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dtemperature, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
