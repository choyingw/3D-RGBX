...
template <uint32_t C, uint32_t TH>
...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_th,
	const float* __restrict__ dL_invdepths,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dtemperature,
...
)
{
	...
	__shared__ float collected_temperature[TH * BLOCK_SIZE];
	...
	float accum_rec_th[TH] = { 0 };
	float dL_dpixel_th[TH];
	float dL_invdepth;
	...
	if (inside)
	{
		...
		for (int i = 0; i < TH; i++)
			dL_dpixel_th[i] = dL_dpixels_th[i * H * W + pix_id];
		if(dL_invdepths)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < TH; i++)
				collected_temperature[i * BLOCK_SIZE + block.thread_rank()] = temperature[coll_id * TH + i];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < TH; ch++)
			{
				//const float th = collected_temperature[ch * BLOCK_SIZE + j];
				// Update last color (to be used in the next iteration)
				//accum_rec_th[ch] = last_alpha * last_temperature[ch] + (1.f - last_alpha) * accum_rec[ch];
				//last_temperature[ch] = th;

				const float dL_dchannel_th = dL_dpixel_th[ch];
				//dL_dalpha += (th - accum_rec_th[ch]) * dL_dchannel_th;
				
				// Update the gradients w.r.t. color of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dtemperature[global_id * TH + ch]), dchannel_dcolor * dL_dchannel_th);
			}
...
void BACKWARD::render(
	...
	const float* dL_dpixels_th,
	const float* dL_invdepths,
	float3* dL_dmean2D,
	float4* dL_dconic2D,
	float* dL_dopacity,
	float* dL_dcolors,
	float* dL_dtemperature,
	float* dL_dinvdepths)
{
	renderCUDA<NUM_CHANNELS, NUM_CHANNELS_THERMAL> << <grid, block >> >(
		ranges,
		point_list,
		W, H,
		bg_color,
