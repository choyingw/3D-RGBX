...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, train_test_exp):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_novel_view")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if idx < len(views) - 10:
            view_next = views[idx + 10]
        elif idx < len(views) - 5:
            view_next = views[idx + 5]
        elif idx < len(views) - 3:
            view_next = views[idx + 3]
        else:
            view_next = views[idx]
