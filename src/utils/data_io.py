...
class DataIOWrapper(nn.Module):
    ...
    def from_cv_imgs_filtering(self, img0, img1, K0=None, K1=None, dist0=None, dist1=None):
        img0_tensor, scale0, mask0, new_K0, img0_undistorted = self.preprocess_image(
            img0, self.device, resize=640, df=self.df, padding=self.padding, cam_K=K0, dist=dist0
        )
        img1_tensor, scale1, mask1, new_K1, img1_undistorted = self.preprocess_image(
            img1, self.device, resize=640, df=self.df, padding=self.padding, cam_K=K1, dist=dist1
        )
        # img0_tensor, scale0, mask0, new_K0, img0_undistorted = self.preprocess_image(
        #     img0, self.device, resize=self.img0_size, df=self.df, padding=self.padding, cam_K=K0, dist=dist0
        # )
        ...
        #     img1, self.device, resize=self.img1_size, df=self.df, padding=self.padding, cam_K=K1, dist=dist1
        # )
        conf_mask, sim_matrix = self.match_images_filtering(img0_tensor, img1_tensor, mask0, mask1)
        data = {"conf_mask": conf_mask, "sim_matrix": sim_matrix}
        return data
    ...
    def from_paths_filtering(self, img0_pth, img1_pth, K0=None, K1=None, dist0=None, dist1=None, read_color=False):

        imread_flag = cv2.IMREAD_COLOR if read_color else cv2.IMREAD_GRAYSCALE

        img0 = cv2.imread(img0_pth, imread_flag)
        img1 = cv2.imread(img1_pth, imread_flag)
        return self.from_cv_imgs_filtering(img0, img1, K0=K0, K1=K1, dist0=dist0, dist1=dist1)

    def from_tensor_filtering(self, img0, img1):
        batch = {"image0": img0, "image1": img1}
        self.model.filtering(batch)
    ...
    def match_images_filtering(self, image0, image1, mask0, mask1):
        ...
        self.model.filtering(batch)
        conf_mask = batch["conf_mask"]
        sim_matrix = batch["sim_matrix"]
        return conf_mask, sim_matrix
